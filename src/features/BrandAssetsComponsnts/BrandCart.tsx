...
const BrandCart: React.FC<PropsWithChildren<IBrandCartProps>> = ({ customStyle, wrapperStyle, children, id }) => {
	...
	return (
		<div data-aos='fade-up' data-aos-delay={`${500 + id * 100}`} className={`relative ${wrapperStyle}`}>
			...
			<div
				...
				className={`relative p-10 h-full flex flex-col gap-4 rounded-2xl cursor-pointer border-2 border-transparent transition-all duration-300 bg-bgDark ${customStyle}`}
				style={gradientStyle}
			>
				{children}
			</div>
